            ...
            <div className="control-panel col-6">
                <div className="add-event-panel">
                    ...
                    <select className="custom-select" id="start" onChange={onSelectChange}
                            ...
                            ref={(input) => {state.calendar.calendar.controlPanel.startInput = input;}}>
                        {calendar.layouts.map((item, key) => {
                            return (
                    ...
                    <select className="custom-select" id="duration" onChange={onSelectChange}
                            ...
                            ref={(input) => {state.calendar.calendar.controlPanel.durationInput = input;}}>
                        {calendar.layouts.map((item, key) => {
                            return (
                                <option key={key} value={item.value}>
                                    {item.title}
                    ...
                    <input type="text" className="form-control" placeholder="Event Title" id="title"
                           onChange={onSelectChange}
                           value={state.calendar.calendar.controlPanel.titleInput.value}
                           ref={(input) => {state.calendar.calendar.controlPanel.titleInput = input;}}/>
